...
function Product() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editId) {
        // UPDATE
        await axios.put(`https://retailiq-1.onrender.com/api/products/${editId}`, form);
        setEditId(null);
      } else {
        // ADD
        await axios.post("https://retailiq-1.onrender.com/api/products", form);
      }
  ...
  const handleDelete = async (id) => {
    await axios.delete(`https://retailiq-1.onrender.com/api/products/${id}`);
    fetchProducts();
  };
